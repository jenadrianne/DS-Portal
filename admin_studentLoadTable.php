...
<body>
      ...
      <tbody align = "center">
		...
		$result = mysqli_query($mysqli, "SELECT p.person_id, CONCAT(p.person_lastname, ' ', p.person_firstname),p.person_schoolId, p.person_phoneNo, s.student_course FROM person p, student s, semester sy WHERE p.person_schoolId = s.person_id AND sy.semester_id =".$q." AND sy.semester_type =".$p);

		if($result){
		  while($row = mysqli_fetch_array($result)){
		    echo "<tr>";
		    echo "<td id='idnumber'>".$row[0]."</td>";
		    echo "<td class='pname'>".$row[1]."</td>";
		    echo "<td class='phone'>".$row[2]."</td>";
		    echo "<td class='sid'>".$row[3]."</td>";
		    echo "<td class='course'>".$row[4]."</td>";
		    echo "<td>";
		    ...
		    echo "<button type='button' title='Delete' data-toggle='modal' data-target='#modalupdate'  data-id='".$row[0]."' class='open-AddBookDialog btn' style='margin-right: 5px; background-color: rgb(155, 183, 190); border-color: rgb(155, 183, 190); color: rgb(255, 255, 255);'><i class='fa fa-pencil'></i></button>";
		     ...
		     echo "<a href='admin_deleteStudents.php?cid=".$row[0]."'><button type='button' title='Delete'  class='btn' style='margin-right: 5px; background-color: rgb(129, 157, 164); border-color: rgb(129, 157, 164); color: rgb(255, 255, 255);'><i class='fa fa-trash-o'></i></button> ";
		    echo "</td>";
		    echo "</tr>";
		  }
		}
